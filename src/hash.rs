...
impl Hash {
    pub fn hash_function(&self) -> HashFunction {
        match self {
            ...
            #[cfg(feature = "sha2")]
            Hash::SHA2_256_Hash(_) => HashFunction::SHA2_256,
            #[cfg(feature = "sha2")]
    ...
    #[cfg(feature = "sha2")]
    pub fn as_sha2_256_hash(&self) -> &SHA2_256_Hash {
        match self {
            Hash::SHA2_256_Hash(hash) => hash,
            #[allow(unreachable_patterns)]
            _ => panic!("programmer error: hash is not SHA2_256_Hash"),
        }
    }
    #[cfg(feature = "sha2")]
    pub fn into_sha2_256_hash(self) -> SHA2_256_Hash {
        match self {
            Hash::SHA2_256_Hash(hash) => hash,
            #[allow(unreachable_patterns)]
            _ => panic!("programmer error: hash is not SHA2_256_Hash"),
...
impl std::fmt::Display for Hash {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            #[cfg(feature = "sha2")]
            Hash::SHA2_256_Hash(hash) => {
                let mut buffer = [0u8; 43];
                write!(
                    f,
                    "{}{}",
                    self.hash_function().keri_prefix(),
                    crate::base64_encode_256_bits(hash.as_ref(), &mut buffer)
                )
            }
            #[cfg(feature = "sha2")]
...
impl std::str::FromStr for Hash {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        const SHA2_256_KERI_PREFIX: &str = HashFunction::SHA2_256.keri_prefix();
        ...
        match &s[..1] {
            ...
            SHA2_256_KERI_PREFIX => {
                #[cfg(feature = "sha2")]
                {
                    let mut buffer = [0u8; 33];
                    let hash_byte_v = crate::base64_decode_256_bits(&s[1..], &mut buffer)?;
                    let hash = SHA2_256_Hash::clone_from_slice(hash_byte_v);
                    Ok(Self::SHA2_256_Hash(hash))
                }
